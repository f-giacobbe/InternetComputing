package it.unical.dimes.reti.traccia2b;
...
public class Communicator {
	...
	static int socketPort; // porta su cui il Communicator rimane in ascolto per
							// ricevere msg TCP

	static void sendMcastDatagram() {
		try {
			while (true) {
				// Invia in multicast un datagram contenente la TCP port
				// (socketPort) usata dall'utente
	...
	public static void main(String args[]) {
		try {
			...
			socketPort = Integer.parseInt(sc.next()); // porta TCP scelta
														// dall'utente
...
class MulticastListener extends Thread {
	...
	int tcpPort; // E' la porta su cui il Communicator è in ascolto per ricevere
	...
	public void run() {
		try {
			...
			while (true) {
				// Riceve datagramma multicast ed estrae da esso l'indirizzo del
				// mittente
				// (remAddress) e la porta del modulo server del mittente
				// (remTCPPort),
				// cioè la porta su cui il mittente è in ascolto per ricevere
				// msg TCP
				byte[] buf = new byte[50];
				...
				while(e.hasMoreElements() && !found)
				{
					...
					{
						InetAddress currAddress = (InetAddress) ee.nextElement();
						if(currAddress.getHostAddress().equals(localhostIP)) {
							found = true;
							break;
						}
					}
